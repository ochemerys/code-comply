...
/**
 * Composable for capturing digital signatures via an HTML canvas (M8-S2).
 *
 * Handles pointer events for cross-device drawing (mouse + touch),
 * tracks empty/drawn state, and exports the signature as image data.
 */
export function useSignature(options?: UseSignatureOptions): UseSignatureReturn {
  ...
  /**
   * Bind the composable to a canvas element. Call once after the element
   * is available (e.g. via a template ref callback or `onMounted`).
   */
    ...
    return new Promise((resolve, reject) => {
      ...
      canvas.value.toBlob((blob) => {
        if (blob) {
          ...
          reject(new Error('Failed to export signature as blob'))
        }
      }, 'image/png')
    })
  }
