...
app.openapi(getGridRoute, async (c) => {
...
app.openapi(postBulkAssignRoute, async (c) => {
  const body = c.req.valid('json')
  try {
...
app.openapi(getWorkloadRoute, async (c) => {
  const q = c.req.valid('query')
